...
class PlgFabrik_Form extends FabrikPlugin
{
    ...
    {
        $params = $this->getParams();
        $condition = $params->get('plugin_condition');
        $process_condition = eval($condition);

        if ((!empty($condition)) && ($process_condition === false)) {
            return false;
        }

        return parent::canUse($location, $event); // TODO: Change the autogenerated stub
    }
}
