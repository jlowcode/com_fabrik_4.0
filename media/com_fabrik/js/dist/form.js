    ...
    var FbForm = new Class({
        ...
        _doPageNav: function (e, dir) {
            ...
            if (this.options.editable) {
                ...
                // Takes the base url (baseUri) and turns it into a array (baseURL) to concatenate with the complementary url (urlComp)
                baseUri = this.form.baseURI;
                baseURL = baseUri.split("/");
                urlComp = 'index.php?option=com_fabrik&format=raw&task=form.ajax_validate&form_id=' + this.id;

                url = baseURL[0] + '//' + baseURL[2] + '/' + urlComp;
                if (this.options.lang !== false) {
                    url += '&lang=' + this.options.lang;
                }
        ...
        _completeValidaton: function (r, id, origid) {
            ...
            if (this.options.toggleSubmit) {
                if (this.options.mustValidate) {
                    ...
                    /**
                     * Begin - Toogle Submit refatory and hidden elements
                     *
                     * Id Task: 71
                     */
                    for (const elNameId in this.mustValidateEls) {
                        exists = document.getElementById(elNameId);
                        if(exists !== null) {
                            //Id Task: 116
                            if(jQuery('#'+elNameId).closest('.fabrikHide').length > 0 || elNameId.indexOf('-auto-complete') >= 0) {
                                this.mustValidateEls[elNameId] = false;
                            }
                        }
                    }
                    ...
                    var validated = true;
                    errors = r.errors;
                    shouldValidate = r.shouldValidate;
                    for(var elName in errors) {
                        for(let repeatCounter = 0; repeatCounter < errors[elName].length; repeatCounter++) {
                            var elForErrors = this.formElements.get(elName);
                            if(elForErrors === null || elForErrors.options.mustValidate === false) {
                                continue;
                            }
                            
                            var fullName = elForErrors.options.fullName;
                            if(fullName != elName) {
                                nameWithRepeat = fullName + '_' + repeatCounter; //For repeatible elements
                            } else {
                                nameWithRepeat = elName;
                            }

                            if(errors[elName][repeatCounter].length > 0) {
                                validated = false;
                                //Toogle Submit default values and dependent fields (Id Task: 72)
                                // Id task: 172
                                if(this.ultraValidated[elName] == true || (this.ultraValidated[elName] == false && fullName != elName)) {
                                    this._showElementError(errors[elName][repeatCounter], nameWithRepeat);
                                    this.ultraValidated[elName] = false;
                                }
                            } else {
                                this.mustValidateEls[nameWithRepeat] = false;
                                this.ultraValidated[elName] = true;
                                if(shouldValidate[elName][repeatCounter] !== false) { // Id task: 167
                                    this._showElementError(errors[elName][repeatCounter], nameWithRepeat);
                                }
                            }
                        }
                    }
                    // End - Toogle Submit refatory and hidden elements

                    if (!this.mustValidateEls.hasValue(true)) {                       
                        this.toggleSubmit(validated);
        ...
        getFormData: function (submit) {
            ...
            this.formElements.each(function (el, key) {
                 ...
                 * Id Task: 68
                 */
                if(el.plugin === 'fileupload') {
                    exists = document.getElementById(key);
                    if(exists !== null) {
                        dataKey = document.getElementById(key).getAttribute('value');
                 ...
                 * Id Task: 71
                 */
                if(jQuery('#'+el.element.id).closest('.fabrikHide').length > 0) {
                    hiddenElements.push(el.element.id);
                }
                //End - Toogle Submit refatory and hidden elements 
            ...
            //Begin - Toogle Submit refatory and hidden elements
            h['hiddenElements'] = JSON.stringify(hiddenElements);
            //End - Toogle Submit refatory and hidden elements 
            return h;
        },
        ...
        duplicateGroup: function (e, scroll) {
             ...
             * Removing the class success when new elements were add in repetible groups
             * 
             * Id task: 169
             */
            this.removeSuccessClass(clone);
            this.removeErrorClass(clone);
            this.removeInfoClass(clone);
                ...
                inputs.each(function (input) {
                    ...
                    if (testid === cloneName || testid === cloneName + '-auto-complete') {
                        ...
                        if (hasSubElements) {
                            ...
                            // Id task: 172
                            if(el.element.id.indexOf('-auto-complete') >= 0) {
                                input.id = bits.join('_') + '-auto-complete';
                            }
        ...
        addMustValidate: function (el) {
            if (this.options.ajaxValidation && this.options.toggleSubmit) {
	            /**
                 * Begin - Toogle Submit in solicitações
                 * Altering the new default for new lines of repetible groups
                 * 
                 * Id task: 170
                 */
                //this.mustValidateEls.set(el.element.id, el.options.mustValidate);
	            this.mustValidateEls.set(el.element.id, el.options.newRepetibleLine);
                if(el.options.newRepetibleLine) {
                    this.toggleSubmit(false);
                }
    			//End - Toogle Submit in solicitações
         ...
         * Id task: 169
         */
        removeSuccessClass: function (element) {
            ...
            for (let i = 0; i < childElements.length; i++) {
                this.removeSuccessClass(childElements[i]);
            }
        },
        ...
        removeErrorClass: function (element) {
            ...
            const childElements = element.children;
            for (let i = 0; i < childElements.length; i++) {
                this.removeErrorClass(childElements[i]);
            }
        },

        removeInfoClass: function (element) {
            if (element.classList.contains("info")) {
                element.classList.remove("info");
                element.classList.remove("has-info");
            }
            
            const childElements = element.children;
            for (let i = 0; i < childElements.length; i++) {
                this.removeInfoClass(childElements[i]);
            }
        }
        // END - Toogle Submit in solicitações
    });
