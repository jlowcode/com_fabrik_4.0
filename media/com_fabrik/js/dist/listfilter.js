...
define(['jquery', 'fab/fabrik', 'fab/advanced-search'], function (jQuery, Fabrik, AdvancedSearch) {
    var FbListFilter = new Class({
        ...
        clearTags: function(){
            var divContainerTags = jQuery('.filteredTags')[0];
            jQuery(divContainerTags).children().each(function (i, f) {
                if(jQuery(f).attr('tag-value')){
                    jQuery(f).remove();
                }
            });
        ...
        clearAFilter: function (f) {
            var sel;
            if (f.prop('name') ? ((f.prop('name').contains('[value]') || f.prop('name').contains('fabrik_list_filter_all'))) : false ||
                f.hasClass('autocomplete-trigger')) {
                if (f.prop('tagName') === 'SELECT') {
                    sel = f.prop('multiple') ? -1 : 0;
                    f.prop('selectedIndex', sel);
                } else {
                    if(f.hasClass('autocomplete-trigger')){
                        if(f.siblings('.autocomplete-trigger').val() != ''){
                            f.siblings('.autocomplete-trigger').val('');
                        }
                    }
                    ...
                    if (f.prop('type') === 'checkbox') {
                        ...
                        //condition added to remove tags and hidden inputs from treeview filter
                        if (f.prop('tree-input-filter') || f.attr('tree-input-filter')) {
                            if (f.attr('data-filter-name')) {
                                f.remove();
                            }
                        }

                        /* for some reason some inputs stay in the front-end, 
                        * so if there are any tags of tree then remove
                        */
                        let tagsContainer = document.getElementsByClassName('tag-container');
                        if (tagsContainer.length != 0) {
                            for (const key in tagsContainer) {
                                if (!tagsContainer.hasOwnProperty(key)) continue;
                                tagsContainer[key].remove();
                            }
                        }

                        //if there are any hidden input of tree then remove
                        let TreeInputHidden = document.getElementsByClassName('pitt_tree_input');
                        if (TreeInputHidden.length != 0) {
                            for (const key in TreeInputHidden) {
                                if (!TreeInputHidden.hasOwnProperty(key)) continue;
                                TreeInputHidden[key].remove();
                            }
                        }
