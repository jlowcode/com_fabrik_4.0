...
define(['jquery', 'fab/encoder', 'fab/fabrik', 'lib/debounce/jquery.ba-throttle-debounce'],
    ...
    var AutoComplete = new Class({
        ...
        search: function (e) {
            ...
            // Begin - Placeholder listener out of table context
            var listeners = this.options.listeners;
            var listen = '';
            if(typeof listeners !== 'undefined' && listeners != "null" && listeners != null) {
                var listening = JSON.parse(listeners);
                listening.forEach(function(elementId) {
                    ...
                    if(element) {
                        ...
                        listen += '&'+ elementId + '=' + value;
                    }
                })
            }
            // End - Placeholder listener out of table context
        ...
        makeSelection: function (e, li) {
            ...
            if (typeOf(li) !== 'null') {
                ...
                if(divFilteredTags && jQuery(divFilteredTags).find("span[tag-value='" + this.element.numValue + "']")[0]){
                    jQuery(divFilteredTags).find("span[tag-value='" + this.element.numValue + "']")[0].remove();
                }

                this.getInputElement().value = li.get('text');
                this.element.value = li.getProperty('data-value');
                this.element.numValue = li.getProperty('data-value');
                this.closeMenu();
                this.fireEvent('selection', [this, this.element.value]);
                // $$$ hugh - need to fire change event, in case it's something like a join element
                // with a CDD that watches it.
                ...
                if(divFilteredEls){
                    jQuery('.filteredTags').append('<span tag-value="'+this.element.value+'" class="tagSearched">' + li.get('text') + '</span>');
                }
                ...
                jQuery('#'+this.element.id+'-auto-complete').trigger('focusout');    // Id task: 172
            } else {
                /**
